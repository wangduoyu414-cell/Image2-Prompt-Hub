...
export default function OperationsPage() {
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-stats"><div><strong>{status?.sources.length ?? "—"}</strong><span>登记来源</span></div><div><strong>{status?.eligible_source_count ?? "—"}</strong><span>持续调度</span></div><div><strong>{status?.review_queue.subject_count ?? "—"}</strong><span>待审核案例</span></div><div><strong>{status?.open_alerts.length ?? "—"}</strong><span>未解决告警</span></div><div><strong>{String(status?.latest_cycle?.state ?? "尚无")}</strong><span>最近周期</span></div></section>
